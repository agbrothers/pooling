...
def load_cifar_10(experiment_path, config):
    ...
    data_path = os.path.join(base_path, "pooling", "datasets", "cifar10")
    trainset = CIFAR10_KFOLD(root=data_path, train=True, transform=transform_train, test_transform=transform_test)
    testset = CIFAR10_KFOLD(root=data_path, train=False, transform=transform_test)
