...
class Scenario(BaseScenario):
    def make_world(
            ...
            num_predators=3, 
            num_obstacles=2, 
            rew_pred_hit_prey=10.0,
            rew_prey_hit_pred=-10.0,
            rew_pred_hit_obstacle=0.0,
            rew_prey_hit_obstacle=0.0,
            collidable_pred=True,
        ...
        self.rew_pred_hit_prey = rew_pred_hit_prey
        self.rew_prey_hit_pred = rew_prey_hit_pred
        self.rew_pred_hit_obstacle = rew_pred_hit_obstacle
        self.rew_prey_hit_obstacle = rew_prey_hit_obstacle
    ...
    def reward(self, agent, world):
        rew_func = self.predator_reward if agent.adversary else self.prey_reward
        return rew_func(agent, world)
    ...
    def prey_reward(self, agent, world):
        ...
        if self.rew_prey_hit_pred != 0:
            pred_min = agent.size + world.agents[0].size
            pred_pos = np.array([a.state.p_pos for a in world.agents if a.adversary])
            pred_dist = np.linalg.norm(pred_pos - agent.state.p_pos, axis=1)
            pred_collisions = pred_dist <= pred_min
            rew += sum(pred_collisions*self.rew_prey_hit_pred)
    ...
    def predator_reward(self, agent, world):
        ...
        if self.rew_pred_hit_prey != 0:
            ...
            rew += sum(prey_collisions*self.rew_pred_hit_prey)
