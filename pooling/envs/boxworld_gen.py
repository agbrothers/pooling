...
def sampling_pairs(num_pair, n=12):
    ...
    for k in range(num_pair):
        key = random.choice(tuple(possibilities))
    ...
    agent_pos = (random.choice(tuple(possibilities)),)
    possibilities -= set(agent_pos)
    first_key = (random.choice(tuple(possibilities)),)
...
def world_gen(n=12, goal_length=3, num_distractor=2, distractor_length=2, seed=None, random_length=True):
    """generate BoxWorld
    """
    if seed is not None:
        random.seed(seed)

    ## RANDOMIZE GOAL AND DISTRACTOR LENGTH UP TO MAX
    if random_length:
        goal_length = random.choice(np.arange(2, goal_length+1))
        num_distractor = random.choice(np.arange(0, num_distractor+1))

    world_dict = {} # dic keys are lock positions, value is 0 if distractor, else 1.
    ...
    distractor_colors = [random.sample(distractor_possible_colors, distractor_length) for _ in range(num_distractor)]
    ...
    for i in range(1, goal_length):
        ...
        world[*keys[i-1]] = color
        world[*locks[i-1]] = colors[goal_colors[i-1]]
        world_dict[tuple(locks[i-1] + np.array([1, 1]))] = 1

    # keys[0] is an orphand key so skip it
    world[*first_key] = colors[goal_colors[0]] if goal_length > 1 else goal_color
    # world_dict[first_key[0]+1, first_key[1]+2] = 1
    ...
    for i, (distractor_color, root) in enumerate(zip(distractor_colors, distractor_roots)):
        ...
        world_dict[key_distractor[0][0] + 1, key_distractor[0][1] + 2] = 0
        for k, key in enumerate(key_distractor[1:]):
            ...
            world_dict[key[0] + 1, key[1] + 2] = 0

    # place an agent
    world[*agent_pos] = np.array(agent_color)
    ...
    return world, agent_pos, world_dict
...
def is_empty(room):
    ...
    # Image.fromarray(world.astype(np.uint8)).save("world.png")
    # Image.fromarray(self.world.astype(np.uint8)).save("world.png")
    # Image.fromarray(img).save("world.png")
...
wall_color = [0, 0, 0]
colors = { 
     0: [  0,   0, 117],  1: [230, 190, 255],  2: [170, 255, 195],  3: [255, 250, 200],  4: [255, 216, 177], 
    ...
    15: [128,   0,   0], 16: [154,  99,  36], 17: [128, 128,   0], 18: [ 70, 153, 144],  
} 
